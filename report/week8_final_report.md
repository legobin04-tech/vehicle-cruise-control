...
## 차량 정속주행 제어기의 설계와 시뮬레이션 기반 성능·강건성 평가

## 1. 보고서 개요

본 프로젝트는 MATLAB과 Simulink를 사용해 차량 종방향 동역학을 모델링하고, PID 기반 Cruise Control을 설계한 뒤 주행 조건을 모사한 물리적 제약과 외란에서 성능을 평가한 8주간의 simulation-based engineering study이다.
...
가장 중요한 전환점은 ±5° 복합 경사 시뮬레이션이었다. 기본 제어기는 오르막에서 크게 감속하고 내리막에서 과속했지만, 외란 구간에서 측정된 구동·제동 요구량은 모델에 설정한 4000 N 한계보다 훨씬 낮았다. 이 근거로 설정한 제어력 한계보다 제어기 게인과 과도응답이 주된 문제라고 판단했고, 제어기를 단계적으로 튜닝했다. 최종 제어기는 동일한 명목 시뮬레이션 조건에서 기본 제어기 대비 외란 RMSE를 50.5%, 초기 오버슈트를 89.5%, 최종 오차를 71.3% 감소시켰다.
...
- 관찰된 성능 저하를 설정한 액추에이터 힘 한계와 제어기 동특성 중 어느 쪽으로 설명할 수 있는가?
- 통제된 게인 실험으로 성능을 개선하면서 액추에이터 여유를 유지할 수 있는가?
- 개선된 설계가 차량 파라미터 변화에서도 유효하며, 그때의 에너지 트레이드오프는 무엇인가?

질문을 시뮬레이션 입력과 파라미터로 변환했고, 결과는 속도 그래프뿐 아니라 RMSE, 최대 편차, 최종 오차, 액추에이터 요구량과 에너지로 정량화했다. 질량 20% 증가, $C_d$ 범위 0.24–0.36, 갑작스러운 경사 변화와 4000 N 제한은 이 연구 질문들에 답하기 위한 실험 변수였다. 이 과정이 프로젝트를 단순 재현에서 engineering study로 바꾼 핵심이다.

취업 포트폴리오 관점에서 이 프로젝트가 보여 주는 것은 특정 툴의 사용법만이 아니다. 물리식의 일관성을 점검하고, 실패 조건을 찾고, 로그를 통해 원인을 분리하고, 통제된 시뮬레이션 비교로 설계 변경의 효과를 확인하는 일련의 문제 해결 과정이다.
...
$$
F_{\mathrm{aero}}=\frac{1}{2}\rho C_d A\,v|v|
$$

여기서 $F_{\mathrm{aero}}$는 속도 부호를 포함한 항력항이다. 따라서 운동방정식의 $-F_{\mathrm{aero}}$는 전진과 후진 모두에서 차량의 운동 반대 방향으로 작용한다.
...
부호 규약은 전진 방향을 양(+)으로 두고, 오르막 경사를 $\theta>0$으로 정의했다. 따라서 $F_{\mathrm{grade}}$는 오르막에서 감속 방향, 내리막에서 가속 방향으로 반영된다. 최종 주행 시험은 $v\geq0$인 전진 영역에서 수행했으므로 $F_{\mathrm{roll}}$은 전진을 방해하는 양의 크기로 모델링해 운동방정식에서 뺐다. 공기항력은 오류 조건에서 속도가 음수가 되더라도 운동 반대 방향으로 작용하도록 $v|v|$ 형태로 구현했다.
...
속도 오차 $e=v_{\mathrm{ref}}-v$를 PID 제어기에 입력해 제어력 $F_{\mathrm{control}}$을 계산했다.

$$
F_{\mathrm{control}}=K_pe+K_i\int e\,dt+K_d\frac{de}{dt}
$$

제어력의 부호에 따라 스로틀과 브레이크를 분리했다.

$$
F_{\mathrm{throttle}}=\max(F_{\mathrm{control}},0)
$$

$$
F_{\mathrm{brake}}=\max(-F_{\mathrm{control}},0)
$$

제어력은 $-4000\leq F_{\mathrm{control}}\leq4000$ N으로 제한했다. 부호 분리 후 스로틀과 브레이크는 각각 0–4000 N 범위를 가지며 동시에 양수가 되지 않는다. 포화 상태에서 적분값이 계속 누적되는 windup을 방지하기 위해 PID 내부 출력 제한과 anti-windup clamping을 적용했다.

비교한 최종 게인은 다음과 같다.

| 제어기 | $K_p$ [N·s/m] | $K_i$ [N/m] | $K_d$ [N·s²/m] |
|---|---:|---:|---:|
| 기본 PID | 500 | 25 | 500 |
| 튜닝 PID | 1000 | 60 | 500 |

게인 단위는 제어기 입력이 속도 오차 [m/s], 출력이 힘 [N]인 본 모델의 정의를 따른다.
...
| Tuned Heavy-Low Cd | 튜닝 | 1800 kg | 0.24 | 큰 관성·작은 자연감속의 복합 시험 조건 |
| Tuned Heavy-High Cd | 튜닝 | 1800 kg | 0.36 | 큰 관성·큰 주행저항의 복합 시험 조건 |

### 4.3 KPI 정의

| KPI | 단위 | 해석 |
|---|---:|---|
| Time to 99% | s | 목표속도의 99%에 처음 도달한 시간 |
| Startup overshoot | m/s | 0–30초 구간의 최대 목표속도 초과량 |
| Overall RMSE | m/s | 0–150초 전체의 대표 추종 오차 |
| Disturbance RMSE | m/s | 30–150초 외란·복귀 구간의 대표 추종 오차 |
| Overall MAE | m/s | 전체 구간 절대 오차의 시간평균 |
| IAE | m | 150초 동안 누적된 절대 오차 |
| Uphill drop | m/s | 오르막에서의 최대 속도 저하량 |
| Downhill rise | m/s | 내리막에서의 최대 초과속도 |
| Return drop | m/s | 평지 복귀 후 최대 속도 저하량 |
| Final error | m/s | 150초 시점의 절대 속도 오차 |
...
$$
MAE=\frac{1}{T}\int_0^T |e(t)|\,dt, \qquad
IAE=\int_0^T |e(t)|\,dt
$$

전체 KPI에는 $T=150$ s를 사용했고, 외란 RMSE에는 30–150 s 구간과 구간 길이 $T=120$ s를 사용했다. 이에 따라 속도 오차의 시간적분인 IAE의 단위는 m이 된다.
...
이 표현은 차량이 전진하는 $v\geq0$ 범위에서는 유효하다. 하지만 $v<0$이 되더라도 $v^2$는 양수이므로, 순힘에서 뺀 공기저항이 차량을 다시 정지 방향으로 밀지 않고 더 음의 방향으로 가속했다. 속도의 절댓값이 커질수록 잘못된 항력과 음의 가속도가 함께 커지며 다음과 같은 양의 피드백이 만들어졌다.
...
$$
F_{\mathrm{aero}}=k v|v|=\frac{1}{2}\rho C_d A\,v|v|
$$
...
### 5.3 모델의 액추에이터 힘 한계 가설 점검

제어력의 크기가 최대 4000 N으로 제한되기 때문에 큰 속도 편차가 액추에이터 포화에서 발생했을 가능성을 먼저 확인했다.
...
두 신호 모두 모델에 설정한 한계보다 충분히 낮았다. 따라서 시뮬레이션의 제어력 한계를 높이는 것은 관찰된 문제의 주원인에 맞는 해결책이 아니었다.
...
$K_p=500 \rightarrow 800 \rightarrow 1000$ 순서로 시험했다. 비례 게인이 커질수록 현재 속도 오차에 더 빠르게 반응하여 오르막 구동과 내리막 제동 전환이 빨라졌다.

$K_p=1000$ 조건은 $K_p=800$ 대비 외란 RMSE를 1.0125 m/s에서 0.8389 m/s로 17.1% 더 줄였고, 최대 속도 편차를 2 m/s 이내로 낮췄다. 반복 진동이나 스로틀·브레이크 채터링도 관찰되지 않아 비례 게인을 1000으로 선정했다.

### 6.2 적분 게인 절충

비례 게인을 선정한 뒤 $K_i=50$, 60, 70을 비교했다.
...
| 최종 오차 | 0.09363 m/s | 0.02233 m/s | 0.01167 m/s |

$K_i=70$은 평균 추종 오차와 최종 오차가 가장 작았지만 내리막과 평지 복귀 순간의 최대 편차 및 액추에이터 요구량이 커졌다. $K_i=50$은 순간 편차가 작지만 오차 제거가 느렸다. 최종적으로 추종 정확도와 순간 최대 편차의 균형을 위해 $K_i=60$을 선택했다.

미분 게인은 기존 $K_d=500$을 유지했다.

## 7. 기본 제어기 대비 최종 성능

![최종 속도 비교](figures/week8_final_speed_comparison.png)

*그림 1. 기본·튜닝 제어기와 두 복합 파라미터 조건의 시뮬레이션 속도 응답. 점선은 30 m/s 기준 속도이며 30, 60, 90 s에서 경사가 전환된다.*
...
튜닝 후 외란 구간 최대 스로틀은 1930.6 N에서 1920.7 N으로, 최대 브레이크는 821.3 N에서 796.9 N으로 소폭 감소했다. 따라서 모델의 최대 제어력 요구량을 높이지 않고 추종 성능을 개선했다.

![기본 제어기 대비 정규화된 성능](figures/week8_normalized_performance.png)

*그림 2. 기본 제어기의 각 KPI를 100%로 정규화한 비교. 표시된 KPI는 모두 값이 낮을수록 추종 성능이 좋다.*
...
앞서 수행한 Week 7 질량 단독 시험에서 기준 차량 대비 다음 변화가 나타났다.
...
시뮬레이션은 오류 없이 완료됐고 최종 속도는 여전히 30 m/s 부근으로 수렴했으므로, 시험 범위에서 정상상태 추종이 유지됐다. 다만 질량 증가는 공기저항 변화보다 응답 시간과 경사 외란 오차에 더 직접적인 영향을 주었다.

### 8.2 공기저항계수가 바뀌면?

앞서 수행한 Week 7 공기저항 단독 시험에서 공기저항계수를 $C_d=0.24$, 0.30, 0.36으로 변화시켰다.
...
최종 비교에서는 두 가지 복합 파라미터 조건을 선택했다.
...
| 최종 오차 | 0.07783 m/s | 0.02559 m/s | 0.02635 m/s |

차량이 20% 무거워진 두 복합 시험 조건에서도 튜닝 제어기는 기본 제어기의 기준 조건보다 우수했다.
...
| 최종 오차 | 67.1% 감소 | 66.1% 감소 |

이 결과는 명목 조건에서 얻은 개선이 시험한 두 복합 파라미터 조건에서도 유지됐음을 보여 준다.

## 9. 구동·제동 에너지 해석

![구동 및 제동 에너지 비교](figures/week8_final_energy_comparison.png)

*그림 3. 네 조건의 바퀴 기준 구동·제동 기계 에너지 비교.*

에너지는 전진 속도만 반영해 다음과 같이 계산했다.

$$
P_{\mathrm{traction}}=F_{\mathrm{throttle}}\max(v,0), \qquad
P_{\mathrm{brake}}=F_{\mathrm{brake}}\max(v,0)
$$

$$
E\,[\mathrm{kWh}]=\frac{1}{3.6\times10^6}\int P(t)\,dt
$$
...
시뮬레이션에서 관찰된 주된 원인은 제어기 동특성이었다. 외란 구간 최대 스로틀 1930.6 N과 최대 브레이크 821.3 N은 모델의 4000 N 한계보다 충분히 낮았다. 반면 +5° 오르막에서 누적된 적분 상태와 구동 명령은 60초의 −5° 내리막 전환 직후에도 영향을 주었고, 제동 전환을 늦춰 과속을 키웠다. 따라서 모델의 제어력 한계를 확대하는 대신 게인과 외란 직후 응답을 개선하는 방향을 선택했다.

### 10.3 액추에이터 요구량을 늘리지 않고 PID 튜닝만으로 얼마나 개선할 수 있는가?

최종 $K_p=1000$, $K_i=60$, $K_d=500$ 제어기는 동일한 명목 시뮬레이션 조건에서 기본 제어기 대비 외란 RMSE를 50.5%, 초기 오버슈트를 89.5%, 최종 오차를 71.3% 줄였다. 동시에 외란 구간 최대 스로틀은 1930.6 N에서 1920.7 N으로, 최대 브레이크는 821.3 N에서 796.9 N으로 소폭 감소했다. 모델의 제어력 한계를 높이지 않고 제어기 조정만으로 추종 성능을 개선했다.

### 10.4 개선은 파라미터 변화에서도 유지되며, 에너지에는 어떤 대가가 생기는가?

차량이 20% 무거워지고 공기저항계수가 동시에 변한 두 복합 시험 조건에서도 튜닝 제어기의 전체 RMSE는 기본 제어기의 명목 조건보다 각각 23.8%, 23.2% 낮았다. 따라서 명목 조건에서의 개선은 시험한 파라미터 범위에서도 유지됐다.

다만 파라미터의 영향은 서로 달랐다. 질량 20% 증가는 외란 RMSE와 최대 제동 요구량을 각각 약 20.2%, 32.6% 증가시켰다. $C_d$ 범위 0.24–0.36의 변화는 전체 속도 RMSE에는 약 ±0.4%만 영향을 주었지만 정상상태 평균 출력과 총 구동 에너지를 각각 약 ±12.4%, ±7.1% 변화시켰다. 또한 튜닝 명목 조건의 구동 에너지는 기본 명목 조건보다 약 2.4% 증가했다. 즉, 속도 추종 개선과 에너지 최소화는 동일한 목표가 아니며 별도의 설계 트레이드오프로 다뤄야 한다.

## 11. 한계와 후속 과제

본 결과는 다음 범위 안에서 해석해야 한다.

- 모든 결과는 MATLAB/Simulink 시뮬레이션에서 얻었으며 실차·HIL·도로 시험은 수행하지 않았다.
- 차량 모델은 종방향 1자유도 모델이며 횡방향·수직방향 동역학은 포함하지 않았다.
- 타이어 슬립, 엔진·모터·변속기 동역학, 센서 노이즈와 지연을 상세히 모델링하지 않았다.
- 구동력과 제동력은 단순화된 이상적 액추에이터로 표현했다.
- 질량·공기저항·경사 조건의 제한된 파라미터 스윕이며, 수학적 안정성 또는 강건성을 증명하거나 모든 실제 주행 조건을 보장하지 않는다.
- 에너지 지표는 바퀴 기준 기계적 값이며 실제 소비전력·연료소비율 또는 연비·전비 개선을 나타내지 않는다.
...
±5° 경사 시뮬레이션에서 큰 속도 편차를 발견했고, 액추에이터 로그를 통해 모델의 4000 N 제어력 한계가 주원인이 아님을 확인했다. 이후 비례·적분 게인을 단계적으로 비교하여 최종 제어기를 선정했고, 동일한 명목 시뮬레이션 조건에서 외란 RMSE 50.5%, 초기 오버슈트 89.5%, 최종 오차 71.3%의 개선을 확인했다. 나아가 차량 질량과 공기저항을 함께 변화시킨 두 복합 조건에서도 시험 범위 안에서 기본 제어기의 명목 조건보다 우수한 성능을 유지했다.
